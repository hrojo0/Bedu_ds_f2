...
data.2$mkt_strategy <- as.logical(data.2$mkt_strategy) #cambiar tipo de dato de int a logical

class(data.2$date)
data.2$date <- as.Date.character(data.2$date, format = "%d/%m/%Y") #cambiar de char a date

class(data.1$ID) #cambiar de int a 
...
visitas.rest.pipe <- visitas.rest %>%
...
visitas.rest.per.visita <- visitas.rest %>%
                           select(date, spending, total) %>%
                           mutate(spending.per.visit = spending / total) %>%
                           arrange(desc(spending.per.visit))
...
visitas.rest.group.summ <- visitas.rest %>%
                           ...
                           summarize(total_spending = sum(spending),
                                     mean_spending = mean(spending), #mean se utiliza para promedio
                                     sd_spending = sd(spending)) #sd es para desviación estándar
head(visitas.rest.group.summ)

df.pba <- as.data.frame(visitas.rest.group.summ)
class(df.pba)
...
g2 <- ggplot(fish, aes(x = Year, y = Capture, linetype = Species)) + 
  geom_line() + theme_minimal()

ggsave("pescados.jpg", plot = g2)
